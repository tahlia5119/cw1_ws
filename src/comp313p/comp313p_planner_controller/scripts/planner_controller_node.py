...
# The planners used to choose the path
...
class PlannerControllerNode(object):
	...
	# Getting the needed parameters to select a planner
	plan = rospy.get_param('planner')
	heuristic = rospy.get_param('heuristic')
	weight = rospy.get_param('weight')
	constant = rospy.get_param('constant')

	if plan == 1:
	    rospy.loginfo('Running FIFO')
	    self.planner = FIFOPlanner('FIFO',self.occupancyGrid)
	elif plan == 2:
       	    rospy.loginfo('Running Best First')
            self.planner = BestPlanner('Best First',self.occupancyGrid)
        elif plan == 3:
	    rospy.loginfo('Running Dijkstra')
	    self.planner = DIJPlanner('Dijkstra\'s',self.occupancyGrid)
        elif plan == 4:
	    rospy.loginfo('Running A*')
  	    self.planner = APlanner('A*', self.occupancyGrid,heuristic,weight,constant)
